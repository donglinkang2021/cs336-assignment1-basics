...
CUDA_VISIBLE_DEVICES=0 uv run train_muon.py \
    ...
    'logger.run_name=leaderboard-qknorm-muon-betas1-gpt2-tie-embed-t20k'
...
CUDA_VISIBLE_DEVICES=0 uv run train_muon.py \
    ...
    optimizer.max_l2_norm=2.0 \
    optimizer.betas="[0.95,0.999]" \
    'logger.run_name=leaderboard-qknorm-muon-betas1-tie-embed'

CUDA_VISIBLE_DEVICES=0 uv run train_muon.py -m \
    +model.add_qknorm=True \
    +model.tie_embeddings=True \
    model.vocab_size=32000 \
    model.num_layers=12 \
    ...
    optimizer.max_l2_norm=2.0 \
    optimizer.betas="[0.95,0.999]" \
    'logger.run_name=leaderboard-qknorm-muon-betas1-gpt2-tie-embed-ws${optimizer.warmup_iters}'
