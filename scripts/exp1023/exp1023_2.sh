WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0 uv run train.py -m \
    ...
    model.ffn_type=vq \
    model.d_model=768 \
    model.d_ff=32,64,128,256,512,1024,2048,3072 \
    optimizer.max_lr=1e-2 \
    optimizer.weight_decay=0.01 \
    optimizer.max_l2_norm=2.0 \
    optimizer.betas="[0.95,0.999]" \
    'logger.run_name=exp1023-ts-vq-dff${model.d_ff}'
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0 uv run train.py \
    ...
    optimizer.max_l2_norm=2.0 \
    optimizer.betas="[0.95,0.999]" \
    'logger.run_name=exp1023-ts-mhvq-nc${model.num_codebook}-cs${model.codebook_size}'
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0 uv run train.py -m \
    ...
    optimizer.max_l2_norm=2.0 \
    optimizer.betas="[0.95,0.999]" \
    'logger.run_name=exp1023-ts-${model.ffn_type}-dff${model.d_ff}'
